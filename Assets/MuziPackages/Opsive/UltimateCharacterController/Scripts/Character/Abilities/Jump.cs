...
namespace Opsive.UltimateCharacterController.Character.Abilities
{
    ...
    public class Jump : Ability
    {
        ...
        [UnityEngine.Serialization.FormerlySerializedAs("m_PrevntSlopeLimitJump")]
        ...
        private void ApplyJumpForce()
        {
            if (IsActive && !m_JumpApplied) {
                ...
                Vector3 direction = m_CharacterLocomotion.Moving ? m_CharacterLocomotion.Up + (m_Transform.forward * 0.25f) : m_CharacterLocomotion.Up;
                AddForce(direction * force, m_Frames, false, true);

                // Ensure the character is in the air after jumping.
                SchedulerBase.ScheduleFixed(Time.deltaTime * 2, EnsureAirborne);

                // Cancel crouch/height change
                m_CharacterLocomotion.TryStopAbility(m_heightChangeAbility);
            }
        }
