...
namespace Opsive.UltimateCharacterController.Character
{
    ...
    public class UltimateCharacterLocomotionHandler : StateBehavior
    {
        ...
        [Tooltip("Auto-run indicator.s")]
        [SerializeField] protected Image m_ImageRunTimer;
        ...
        private SpeedChange m_SpeedChangeAbility;
        private CanvasGroup m_ImageRunTimerCanvasGroup;
        ...
        protected override void Awake()
        {
            ...
            if (m_ImageRunTimer != null)
            {
                m_ImageRunTimerCanvasGroup = m_ImageRunTimer.GetComponent<CanvasGroup>();
            }
        ...
        private void Update()
        {
            ...
            // autorun related
            if (m_PlayerInput.GetButton("Change Speeds") || m_SpeedChangeAbility.isAutoRun)
            {
                if (m_ForwardMovement == 0 && m_HorizontalMovement == 0)
                    if (m_SpeedChangeAbility.runDuration >= 0.2f)
                        m_ForwardMovement = 1;

                if (!m_PlayerInput.GetButton("Change Speeds") && m_SpeedChangeAbility.isAutoRun && (m_PlayerInput.GetAxisRaw(m_HorizontalInputName) != 0 || m_PlayerInput.GetAxisRaw(m_ForwardInputName) != 0))
                {
                    m_CharacterLocomotion.TryStopAbility(m_SpeedChangeAbility);
                    m_ForwardMovement = 0;
                }
            }
            else if (m_HorizontalMovement == 0 && m_ForwardMovement == 0)
                    m_CharacterLocomotion.TryStopAbility(m_SpeedChangeAbility);

            if (m_ImageRunTimer != null)
            {
                if (m_SpeedChangeAbility.runDuration >= 0.5f && !m_SpeedChangeAbility.isAutoRun)
                {
                    m_ImageRunTimerCanvasGroup.alpha = 0.5f;
                    m_ImageRunTimer.gameObject.SetActive(true);
                    m_ImageRunTimer.fillAmount = Mathf.Clamp01((m_SpeedChangeAbility.runDuration-0.5f) / (autoRunTime - 0.5f));
                }
                else
                {
                    if (m_SpeedChangeAbility.isAutoRun)
                    {
                        m_ImageRunTimer.fillAmount = 1;
                        m_ImageRunTimerCanvasGroup.alpha = 0.9f;
                    }
                    else m_ImageRunTimer.gameObject.SetActive(false);
                }
            }
            //
