...
namespace Opsive.UltimateCharacterController.Character
{
    ...
    public class CharacterIK : CharacterIKBase
    {
        ...
        public bool InitializeBones(bool fromAwake, bool force = false)
        {
            if (force)
            {
                m_Head = null;
            ...
            charFrontFacingTransform = transform;//m_Animator.GetBoneTransform(HumanBodyBones.Hips);
        ...
        private void OnAttachLookSourceNotFromAwake(ILookSource lookSource)
        {
            var prevEnabled = enabled;
            m_LookSource = lookSource;
        ...
        public override Vector3 GetDefaultLookAtPosition()
        {
            ...
            //anh nguyen: flip camera lookAt when face is facing camera ( for artist request)
            if (cameraTransform == null)
            {
                return MathUtility.TransformPoint(headPosition, Quaternion.LookRotation(lookDirection), localOffset);
            }

            var faceCameraVector = (cameraTransform.position - charFrontFacingTransform.position );
            var projectedOnYPlane = Vector3.ProjectOnPlane(faceCameraVector, Vector3.up).normalized;
            
            var halfViewAngle = viewHorizontalAngle/2f;
            var dotProduct = Vector3.Dot(projectedOnYPlane, charFrontFacingTransform.forward);
            var isCameraInCharacterFrontView = dotProduct > Mathf.Cos(Mathf.Deg2Rad * halfViewAngle);
            // Debug.Log($"<color=red>Is isCameraInCharacterFrontView: {Vector3.Dot(projectedOnYPlane, characterFrontFacingTF.up) }</color>");
            // due to the animation rig exported coordinate, the bone UP in unity is the look direction instead of Forward (characterFrontFacingTF.Forward)
            if (_previousViewState != isCameraInCharacterFrontView)
            {
                // Debug.Log($"<color=yellow>Change view state in character front view from {_previousViewState} to {isCameraInCharacterFrontView}</color>");
                _previousViewState = isCameraInCharacterFrontView;
                _justChangeViewState = true;
                // keep the prev lookDir
                _directionBeforeTurningHead = _adjustedLookDirection;
                _elapsedTime = 0f;
            }

            if (!isCameraInCharacterFrontView) // limit look direction in case of camera not in character front view area
            {
                var projectedOnYPlane2 = Vector3.ProjectOnPlane(lookDirection, Vector3.up).normalized;
                // Debug.Log($"<color=red>Projected on Y plan {projectedOnYPlane2} and forward {charFrontFacingTransform.forward}</color>");
                // Debug.Log($"<color=red>sine (forward, loo) {Vector3.sin}</color>");
                var angleWithStraightView = Vector3.SignedAngle(projectedOnYPlane2, charFrontFacingTransform.forward, Vector3.up);
                // Debug.Log($"<color=yellow>Direction angle with straight view {angleWithStraightView}</color>");
                var outOfWideLookView = Mathf.Abs(angleWithStraightView) > halfViewAngle;
                if (outOfWideLookView)
                {
                    var adjustAngle = angleWithStraightView > 0 ? angleWithStraightView - halfViewAngle : angleWithStraightView + halfViewAngle;
                    // Debug.Log("<color=yellow>\tIT already out of front view</color>");
                    // Debug.Log($"<color=yellow>\tadjust angle {adjustAngle}</color>");
                    // Debug.Log($"<color=yellow>\tLook direction before {lookDirection}</color>");
                    lookDirection = Quaternion.Euler(0f, adjustAngle, 0f) * lookDirection;
                    
                    // Debug.Log($"<color=yellow>\tLook direction after adjust {lookDirection}</color>");
                }
            }
            
            
            var flipped = isCameraInCharacterFrontView ? -1 : 1;
            
            var clampedYLookDirection = lookDirection;
            clampedYLookDirection.y = Mathf.Clamp(clampedYLookDirection.y, -maxHeadUpDownRange, maxHeadUpDownRange);
            if (_justChangeViewState) // lerp the avoid-camera head rotation
            {
                _elapsedTime += turnAvoidCameraSpeed * Time.deltaTime;
                _adjustedLookDirection = Vector3.Lerp(_directionBeforeTurningHead, flipped * clampedYLookDirection, _headTurnTween.Evaluate(_elapsedTime));
                if (_elapsedTime >= 1f)
                {
                    _elapsedTime = 0f;
                    _justChangeViewState = false;
                }
            }
            else
            {
                _adjustedLookDirection = flipped * clampedYLookDirection;
            }

            // Debug.Log($"<color=red>Direction fetched {_adjustedLookDirection}</color>");
            return MathUtility.TransformPoint(headPosition, Quaternion.LookRotation(_adjustedLookDirection), localOffset);
        }

        [Header("Muziverse Head Turn")]
        [SerializeField] private Transform charFrontFacingTransform;
        [SerializeField] private Transform cameraTransform;
        [Space(10f)]
        [Tooltip("Character front horizontal view angle limit")]
        [SerializeField] private float viewHorizontalAngle = 120f;
        [Tooltip("Character vertical view angle (0.2f..)")]
        [SerializeField] private float maxHeadUpDownRange = 0.15f;
        [SerializeField] private float turnAvoidCameraSpeed = 1.2f;
        [SerializeField] private AnimationCurve _headTurnTween = new AnimationCurve();

        // other local variable to support turning logics
        private bool _justChangeViewState = false;
        private float _elapsedTime = 0f;
        Vector3 _adjustedLookDirection;
        private Vector3 _directionBeforeTurningHead;
        private bool _previousViewState = false;
        ...
        {
#if UNITY_EDITOR
            if (charFrontFacingTransform != null) // facing direction
            {
                Gizmos.color = Color.magenta;
                Gizmos.DrawLine(charFrontFacingTransform.position, charFrontFacingTransform.position + charFrontFacingTransform.up * 15f);
                
                Gizmos.color = Color.blue;
                if (cameraTransform != null)
                {
                    Gizmos.DrawLine(charFrontFacingTransform.position, cameraTransform.position);
                }
            }
#endif
        }
        ...
        private void LookAtTarget()
        {
            ...
            // If the target position has a positive y value then the character should not look down between the lerp.
            ...
            m_LookAtPosition.y = Mathf.Clamp(m_LookAtPosition.y, headPosition.y - 0.24f, headPosition.y + 0.24f);
        ...
        {
            EventHandler.UnregisterEvent<ILookSource>(m_GameObject, "OnCharacterAttachLookSource", OnAttachLookSource);
            EventHandler.RegisterEvent<ILookSource>(m_GameObject, "OnCharacterAttachLookSource", OnAttachLookSourceNotFromAwake);
        }
    }
}