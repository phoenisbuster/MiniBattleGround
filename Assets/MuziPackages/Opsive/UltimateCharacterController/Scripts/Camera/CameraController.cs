...
namespace Opsive.UltimateCharacterController.Camera
{
    ...
    public class CameraController : StateBehavior, ILookSource, Shared.Camera.ICamera
    {
        ...
        IEnumerator Start()
        {
            if (m_Character != null) {
                if (m_InitCharacterOnAwake) {
                    ...
                    bool lookattargetready = false;
                    while (!lookattargetready && character != null)
                    {
                        var ik = character.GetComponent<CharacterIK>();
                        if (ik != null)
                        {
                            lookattargetready = ik.LookatTargetReady;
                        }
        ...
        public void SetCameraState(string stateName, bool stateStatus)
        {
            if (stateName != string.Empty)
            {
                StateManager.SetState(m_GameObject, stateName, stateStatus);
                StateManager.SetState(m_Character, stateName, stateStatus);
            }
            ...
            //m_RangeStateName = stateStatus ? stateName : "";
        }

        /// <summary>
        /// Subscribe this function to OnZoomEvent on Editor
        /// </summary>
        public void OnZoomWhenCameraOnCustomRange(bool zoomStatus)
        {
            if (m_RangeStateName != string.Empty)
            {
                StateManager.SetState(m_GameObject, m_RangeStateName, zoomStatus);
                StateManager.SetState(m_Character, m_RangeStateName, zoomStatus);
            }
        }
