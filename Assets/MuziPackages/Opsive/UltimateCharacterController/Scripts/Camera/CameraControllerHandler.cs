...
namespace Opsive.UltimateCharacterController.Camera
{
    ...
    {
        [Tooltip("Is the zoom enable?")]
        [SerializeField] protected bool m_EnableZoom = true;
        ...
        private float[] m_PresetDistanceLevel = new float[4] { 4f, 2.8f, 1.6f, 0.4f };  // Y Anchor Offset is 1.55f
        ...
        private void Update()
        {
            ...
            //UpdateCameraState(Vector3.Distance(transform.position, m_Character.transform.position));
        }

        public void UpdateCameraState(string stateName, bool status)
        {
            m_CameraController.SetCameraState(stateName, status);
        }
